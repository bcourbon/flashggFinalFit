...
parser.add_option("-A","--Abslimit",dest="Abslimit",default=False,action="store_true",help="Do abs limit plot")  #FAN
...
def drawGlobals(canv,shifted=False):
  ...
  if shifted: 
   ...
   lat.DrawLatex(0.152,0.93,"CMS Preliminary")   #FAN
...
def pvalPlot(allVals):
  ...
  for i,sig in enumerate(sigmas):
    ...
    lines[i].SetLineColor(2) # red Lines FAN
    #labels.append(r.TLatex(150+1, y * 0.8, "%d #sigma" % (i+1)))
    labels.append(r.TLatex(float(options.xaxis.split(',')[0]) + 25, y * 1.1 , "%d #sigma" % (i+1)))  #FAN
    #labels[i].SetTextColor(13)
    labels[i].SetTextColor(1)   #FAN
    labels[i].SetTextSize(0.04)   #FAN
  ...
  canv.Print('%s.pdf'%options.outname)  #FAN
  canv.Print('%s.png'%options.outname)
  #canv.Print('%s.C'%options.outname)   #FAN
...
def limitPlot(allVals):
  ...
  buildSMHiggsSignalXSBR = imp.load_source('*', '../AnalysisScripts/python/buildSMHiggsSignalXSBR.py')  #FAN
  buildSMHiggsSignalXSBR.Init8TeV() #FAN
  ...
  for k, values in enumerate(allVals):
    ...
    for j in range(len(values)):
      if (j%ents_per_mass==0):
        ...
        #FAN
        if options.Abslimit: 
            valBR = buildSMHiggsSignalXSBR.getBR(mh)
            valXSggH = buildSMHiggsSignalXSBR.getXS(mh,"ggH")
            valXSqqH = buildSMHiggsSignalXSBR.getXS(mh,"qqH") 
            valXSttH = buildSMHiggsSignalXSBR.getXS(mh,"ttH")
            valXSWH = buildSMHiggsSignalXSBR.getXS(mh,"WH")
            valXSZH = buildSMHiggsSignalXSBR.getXS(mh,"ZH")
            #valSMAbs = (valXSggH+valXSqqH+valXSttH+valXSWH+valXSZH)*valBR  #all
            #valSMAbs = (valXSggH)*valBR  #ggh only
            #valSMAbs = (valXSqqH+valXSttH+valXSWH+valXSZH)*valBR  #no ggh
            #SGS 
            #valSMAbs = (valXSggH+valXSttH)*valBR  #ggh+ttbarh
            valSMAbs = (valXSqqH+valXSWH+valXSZH)*valBR  #vbf+ggh
	    #valSMAbs = (valXSqqH+valXSWH+valXSZH)*valBR  #vbf+VH
            #valSMAbs = (valXSqqH+valXSWH+valXSZH)*valBR  #vbf+VH
            #SGS

            down95 = down95*valSMAbs
            down68 = down68*valSMAbs
            median = median*valSMAbs
            up68 = up68*valSMAbs
            up95 = up95*valSMAbs
        #FAN
    ...
    if len(allVals)==1:
      ...
      mg.Add(oneSigma)
    mg.Add(exp)
    if not options.expected: mg.Add(graph)   #FAN
  
  # draw dummy hist and multigraph
  #dummyHist.GetYaxis().SetTitle("\sigma(H#rightarrow #gamma #gamma)_{95%%CL} / \sigma(H#rightarrow #gamma #gamma)_{SM}")
  dummyHist.GetYaxis().SetTitle("#sigma(H#rightarrow #gamma #gamma)_{95%CL} / #sigma(H#rightarrow #gamma #gamma)_{SM}")  #FAN
  #FAN
  if options.Abslimit:
      dummyHist.GetYaxis().SetTitle("#sigma #times Br(H#rightarrow #gamma #gamma)_{95%CL}(pb)/GeV")
      dummyHist.GetYaxis().SetTitleOffset(1.4)
  #FAN
  mg.Draw("A")
  if not options.yaxis:
    dummyHist.SetMinimum(mg.GetYaxis().GetXmin())
...
def plotMPdfChComp():
  ...
  if options.chcompLine:
    ...
    label1=r.TText()
    label1.SetNDC()
    label1.SetText(0.26,0.4,"8 TeV")
    label1.SetTextAngle(90)
    label2=r.TText()
    label2.SetNDC()
    label2.SetText(0.26,0.75,"7 TeV")
